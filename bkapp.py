...
def onboarding_popup():
    ...
    with st.form("onboarding_form"):
        ...
        if submitted:
            ...
            # 建立空的 user_profile 字典 (這就是解決您之前當機的關鍵！)
...
if st.session_state.logged_in:
    ...
    # 檢查是否為新用戶
    if "user_profile" not in st.session_state or st.session_state.get("is_new_user", False):
        st.session_state.is_new_user = True
        onboarding_popup() # 呼叫彈跳視窗 (現在它找得到這個函式了！)
    else:
        show_member_app() # 老朋友，直接進去戰情室
        
else:
    # 🛑 乾淨的 V20.34 登入頁面 UI (絕對沒有 Email 登入)
    ...
    with col2:
        ...
        if auth_url:
            st.markdown(f'''
                <a href="{auth_url}" target="_self" style="background-color:#06C755; color:white; padding:15px; display:block; text-align:center; text-decoration:none; border-radius:10px; font-weight:bold;">
                    LINE 帳號登錄 / 註冊
                </a>
