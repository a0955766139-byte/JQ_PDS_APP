...
def render():
    ...
    if selected_profile == "NEW" or (mode == "我的本命盤" and selected_profile is None):
        ...
        with st.container(border=True):
            ...
            bd = st.date_input("出生日期", datetime.date(1983, 9, 8))
            if st.button("🚀 啟動導航", type="primary", use_container_width=True):
                _save_chart(username, name, eng, bd, is_me=(mode == "我的本命盤"))
                st.rerun()
    elif selected_profile:
        p = selected_profile
        bd = datetime.datetime.strptime(p['birth_date'], "%Y-%m-%d").date() if isinstance(p['birth_date'], str) else p['birthdate']
